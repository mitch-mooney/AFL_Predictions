...
# Get Football Draw
fixture <- fetch_fixture_squiggle(season = 2022, round_number = 1)
fixture %<>%
  rename(Date = date,
         Season = year,
         Season.Game = id,
...
dat.new<-fetch_player_stats_footywire(season = 2021, round_number = 27, check_existing = TRUE) %>% 
  filter(grepl('Grand Final', Round)) %>% mutate(Date = as.Date(Date, format = "%Y-%m-%d"))
