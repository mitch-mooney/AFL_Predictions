...
# Get Football Draw
fixture <- fetch_fixture_squiggle(season = 2021, round_number = 23)
fixture %<>%
  rename(Date = date,
         Season = year,
         Season.Game = id,
...
dat.new<-fetch_player_stats_footywire(season = 2021, round_number = 22, check_existing = TRUE) %>% 
  filter(Round == "Round 22") %>% mutate(Date = as.Date(Date, format = "%Y-%m-%d"))
...
round <- wrangle_fixture(round = 23)
