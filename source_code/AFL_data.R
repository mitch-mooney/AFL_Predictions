...
# Get Football Draw
fixture <- fetch_fixture_squiggle(season = 2022, round_number = round)
fixture %<>%
  rename(Date = date,
         Season = year,
         Season.Game = id,
...
dat.new<-fetch_player_stats_footywire(season = 2022, round_number = round-1, check_existing = TRUE)  %>% 
...
round <- wrangle_fixture(round = round)
