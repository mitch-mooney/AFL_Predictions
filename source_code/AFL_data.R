...
dat.new<-fetch_player_stats_footywire(season = 2021, round_number = 12, check_existing = TRUE) %>% 
  filter(Round == "Round 12") %>% mutate(Date = as.Date(Date, format = "%Y-%m-%d"))
...
round <- wrangle_fixture(round = 13)
