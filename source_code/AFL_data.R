...
dat.new<-fetch_player_stats_footywire(season = 2021, round_number = 13, check_existing = TRUE) %>% 
  filter(Round == "Round 13") %>% mutate(Date = as.Date(Date, format = "%Y-%m-%d"))
...
round <- wrangle_fixture(round = 14)
